...
public class Communicator {
	...
	private static String appKey = "f8c31e98eae272b17f08498d99497184";
	...
	public Communicator(Context _context) {
		...
		setApplicationAttributes(applicationPreferences.getString("APP_KEY", appKey), 
				applicationPreferences.getString("SERVER_ADDRESS", server));
	}
	
	private void setApplicationAttributes(String _appKey, String serverAddress){
		appKey = _appKey;
	...
	public boolean registerDevice(String appKey, String imei, String deviceName){
		...
		sendMap.put(KEY_IMEI, imei);
		sendMap.put(KEY_NAME,deviceName);
...
private void addRemoteIdentificationInfo(HashMap<String, String> sendMap) {
	...
	public ContentValues getRemoteFindById(int remoteFindId) {
		...
		String responseString = doHTTPPost(RECEIVE_FINDS_URL, sendMap);
		try {
			HashMap<String, Object> responseMap = (new ResponseParser(
					responseString).parse());
			if (PositHttpUtils.isResponseOK(responseMap)) {
				...
				if (finds.length()>0) {
					HashMap<String,Object> rMap = new ResponseParser(
						finds.getString(0)).parse();
					cleanupOnReceive(rMap);
