...
/**
 * An interface for wordwrap scanner.
 * @author pgautam
 *
 */
...
class FontMetricsScanner implements WordWrapScanner
    {
    float metrics;
    public FontMetricsScanner(float pixels)
        {
        this.metrics = pixels;
        }
    ...
    public int scan(StringBuffer buf, int start, double nextLoc)
        ...
        for(int x = 0; x < chars.length; x++)
            ...
            int len = (int)((x+2)*metrics); /*changed from using charsWidth to just 
            								getting the numbers and multiplying by the
            								font size*/
...
public class WordWrap implements java.io.Serializable
    ...
    public static String wrap(String string, int numPixels, float metrics)
        {
        return wrap(string, numPixels, metrics);
        }
