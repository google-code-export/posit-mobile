...
public class MyDBHelper extends SQLiteOpenHelper {
	 ...
	 * @param values  containts the <column_name,value>  pairs for each DB field.
	 * @return the row ID of the newly inserted row, or -1 if an error occurred
	 */
	...
	public boolean deleteVideo(long videoId) {
		mDb = getWritableDatabase();
		boolean result = mDb.delete(PHOTO_TABLE_NAME, KEY_ID + "=" + videoId, null) > 0;
		mDb.close();
		return result;
	}
	...
	public boolean deletePhotoByPosition(long findId, int position) {
		mDb = getWritableDatabase();
		Cursor images = mDb.query(PHOTO_TABLE_NAME, new String[]{KEY_ID}, KEY_FIND_ID+"="+findId, null, null, null, null);
	...
	public boolean deleteVideoByPosition(long findId, int position) {
		mDb = getWritableDatabase();
		Cursor videos = mDb.query(VIDEO_TABLE_NAME, new String[]{ KEY_ID }, KEY_FIND_ID + "=" + findId, null, null, null, null);
	...
	public boolean deleteAudioByPosition(long findId, int position) {
		mDb = getWritableDatabase();
		Cursor audios = mDb.query(AUDIO_TABLE_NAME, new String[]{ KEY_ID }, KEY_FIND_ID + "=" + findId, null, null, null, null);
		audios.moveToPosition(position);
		long audioId = audios.getLong(audios.getColumnIndex(KEY_ID));
		mDb.close();
		audios.close();
		return deleteVideo(audioId);
	}
	...
	public Cursor getVideosCursor(long id) {
		mDb = getReadableDatabase(); 
		String[] columns = { KEY_FIND_ID, KEY_VIDEO_ID, KEY_VIDEO_URI };
		String[] selectionArgs = null;
		String groupBy = null, having = null, orderBy = null;
	...
	public Cursor getAudiosCursor(long id) {
		mDb = getReadableDatabase(); 
		String[] columns = { KEY_FIND_ID, KEY_AUDIO_ID, KEY_AUDIO_URI };
		String[] selectionArgs = null;
		String groupBy = null, having = null, orderBy = null;
