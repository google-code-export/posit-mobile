...
public class ServerRegistrationActivity extends Activity {
	...
	private static final int SANDBOX = 1;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		sandboxButton.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
					...
					if (true) {
						TelephonyManager manager = (TelephonyManager) ServerRegistrationActivity.this
								.getSystemService(Context.TELEPHONY_SERVICE);
						String imei = manager.getDeviceId();
						String authKey = "sb_" + imei;
						String sandbox = "http://posit.hfoss.org/sandbox";
						Communicator communicator = new Communicator(
								ServerRegistrationActivity.this);
						boolean registered = communicator.registerDevice(
								sandbox, authKey, imei);
						if (registered) {
							SharedPreferences sp = PreferenceManager
									.getDefaultSharedPreferences(ServerRegistrationActivity.this);
							Editor spEditor = sp.edit();

							spEditor.putString("SERVER_ADDRESS", sandbox);
							spEditor.putString("AUTHKEY", authKey);
							spEditor.putInt("PROJECT_ID", 4); // sandbox project
																// id
							spEditor.putString("PROJECT_NAME",
									"Sandbox Project");
							spEditor.commit();
							Intent intent = new Intent(ServerRegistrationActivity.this, SyncActivity.class);
							intent.setAction(Intent.ACTION_SYNC);
							startActivity(intent);
							finish();
						} else {
							Utils
									.showToast(ServerRegistrationActivity.this,
											"Error. Make sure you have an internet connection.");
						}
					}
