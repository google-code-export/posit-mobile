...
public class CameraPreview extends Activity {
	...
	//static private Camera cameraObject = new Camera();
	static private Camera.Parameters param = Camera.open().getParameters();
...
*/	
	param.setPreviewSize(dm.widthPixels, dm.heightPixels);
 ...
 */
	...
	protected void onResume() {
		// Because the Camera object is not a shared resource,
 ...
 */
	...
	private void takePicture() {
		...
		if (camera != null) {
			...
			Bitmap myPic = Bitmap.createBitmap(param.getPictureSize().width,
					param.getPictureSize().height, Bitmap.Config.RGB_565);
	...
	class Preview extends SurfaceView implements SurfaceHolder.Callback {
		...
		private Camera.Parameters parama = null;

		Preview(Context context, Camera.Parameters param) {
			super(context);
		...
		class PreviewThread extends Thread {
			...
			@Override
			public void run() {
				// We first open the Camera and configure it.
				Camera camera = Camera.open();
				if (camera != null) {
					camera.setParameters(param);
				}
