...
public class Find {
	 ...
	 * @param videos list of contentvalues containing the video references to add
	 * @param audioClips list of contentvalues containing the audio clip references to add
	...
	public boolean delete() {
		if (mDbHelper.deleteFind(mId)) {
			return deleteImages(mId) && deleteVideos() && deleteAudioClips();
		}else 
			return false;
	}
	...
	public Cursor getAudioClips() {
		return mDbHelper.getAudiosCursor(this.mId);
	}
	...
	private boolean deleteAudioClips() {
		if ((audioClips=getAudioClips()) != null) {
