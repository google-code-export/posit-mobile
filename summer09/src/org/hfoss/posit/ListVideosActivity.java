...
public class ListVideosActivity extends ListActivity implements OnCreateContextMenuListener {
	...
	private void fillDataFromDb() {
		...
		String[] columns = { MyDBHelper.COLUMN_VIDEO_URI };
	...
	private void populateMapList(int length) {	
		...
		for (int i = 0; i < length; i++) {
			...
			map.put(MyDBHelper.COLUMN_VIDEO_URI, getIntent().getStringExtra(String.valueOf(i)));
	...
	protected void onListItemClick(ListView listView, View view, int position, long id) {
		...
		case FindActivity.STATE_INSERT:
			...
			uri = Uri.parse(data.get(MyDBHelper.COLUMN_VIDEO_URI).toString());
			if (uri != null) {
				intent.setData(uri);
				startActivity(intent); 
			}
		...
		case R.id.confirm_delete_video_audio: //context menu call; delete specific file
			...
			case FindActivity.STATE_INSERT:
				...
				File file = new File(Utils.getFilenameFromMediaUri(this,
						Uri.parse(mTempVideoMaps.get(info.position).get(MyDBHelper.COLUMN_VIDEO_URI))));
				file.delete();
				...
				for (int i = 0; i < mTempVideoMaps.size(); i++) {
					resultIntent.putExtra(String.valueOf(i), mTempVideoMaps.get(i).get(MyDBHelper.COLUMN_VIDEO_URI).toString());
				}
				...
				intent.putExtra("length", mTempVideoMaps.size());
				for (int i = 0; i < mTempVideoMaps.size(); i++) {
					intent.putExtra(String.valueOf(i), mTempVideoMaps.get(i).get(MyDBHelper.COLUMN_VIDEO_URI).toString());
				}
					...
					case FindActivity.STATE_INSERT:
						while (!mTempVideoMaps.isEmpty()) {
							String filename = Utils.getFilenameFromMediaUri(ListVideosActivity.this,
									Uri.parse(mTempVideoMaps.getFirst().get(MyDBHelper.COLUMN_VIDEO_URI).toString()));
