 ...
 * @author Khanh Pham
 */
public class ListAudioClipsActivity extends ListActivity implements OnCreateContextMenuListener {
	...
	private LinkedList<HashMap<String, String>> mTempAudioMaps;

	/** 
	 * This method is invoked when the user clicks on the view audio clips button on the view
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mTempAudioMaps = new LinkedList<HashMap<String, String>>();
	...
	private void fillDataFromDb() {
		...
		String[] columns = { MyDBHelper.COLUMN_AUDIO_URI };
		int[] views = { R.id.audio_uri_id };
	...
	private void fillDataFromList() {
		...
		SimpleAdapter adapter =
			new SimpleAdapter(this, mTempAudioMaps, R.layout.list_row_audio, columns, views);
		setListAdapter(adapter);
	...
	private void populateMapList(int length) {	
		...
		for (int i = 0; i < length; i++) {
			Log.i(TAG, "inserting #" + i + " audio into mTempAudioMaps");
			Log.i(TAG, "uri: " + getIntent().getStringExtra(String.valueOf(i)));
			Log.i(TAG, "=====================");
			HashMap<String, String> map = new HashMap<String, String>();
			map.put(MyDBHelper.COLUMN_AUDIO_URI, getIntent().getStringExtra(String.valueOf(i)));
			mTempAudioMaps.add(map);
		}
	}
	...
	/**
	 * This method executes when the user clicks on one of the audio clips in the
	 ...
	 *   @param id is the audio clip's RowID
	 */
	@Override
	protected void onListItemClick(ListView listView, View view, int position, long id) {
		...
		case FindActivity.STATE_EDIT:
			...
			Uri uri = mDbHelper.getAudioUriByPosition((int) getIntent().getLongExtra("rowId", -1), position);
		...
		case FindActivity.STATE_INSERT:
			...
			uri = Uri.parse(data.get(MyDBHelper.COLUMN_AUDIO_URI).toString());
			if (uri != null) {
				intent.setData(uri);
				startActivity(intent); 
			}
	...
	public void onCreateContextMenu(ContextMenu menu, View v, ContextMenu.ContextMenuInfo menuInfo){
		...
		menu.setHeaderTitle(this.getResources().getString(R.string.alert_dialog_delete_audio));
	}
	...
	public boolean onMenuItemSelected(int featureId, MenuItem item) {
		...
		case R.id.delete_all_audio_clips: //options menu call; delete all audio clips
		...
		case R.id.confirm_delete_video_audio: //context menu call; delete specific file
			...
			case FindActivity.STATE_EDIT:
				...
				numberOfAudiosLeft = mDbHelper.getAudiosCursor((int) getIntent().getLongExtra("rowId", -1)).getCount();
				if (numberOfAudiosLeft == 0){
					...
					intent.putExtra("length", mTempAudioMaps.size());
					startActivity(intent);
					return true;
				}
			...
			case FindActivity.STATE_INSERT:
				...
				File file = new File(Utils.getFilenameFromMediaUri(this,
						Uri.parse(mTempAudioMaps.get(info.position).get(MyDBHelper.COLUMN_AUDIO_URI))));
				file.delete();

				//remove file from list
				mTempAudioMaps.remove(info.position);

				//create intent to be set as result
				final Intent resultIntent = new Intent(ListAudioClipsActivity.this, FindActivity.class);
				resultIntent.putExtra("length", mTempAudioMaps.size());
				for (int i = 0; i < mTempAudioMaps.size(); i++) {
					resultIntent.putExtra(String.valueOf(i), mTempAudioMaps.get(i).get(MyDBHelper.COLUMN_AUDIO_URI).toString());
				}
				...
				intent.putExtra("length", mTempAudioMaps.size());
				for (int i = 0; i < mTempAudioMaps.size(); i++) {
					intent.putExtra(String.valueOf(i), mTempAudioMaps.get(i).get(MyDBHelper.COLUMN_AUDIO_URI).toString());
				}
		...
		case CONFIRM_DELETE_DIALOG:
			...
			.setPositiveButton(R.string.alert_dialog_ok, 
				...
				public void onClick(DialogInterface dialog, int whichButton) {
					// User clicked OK so delete audio clips
					switch (mState) {
					case FindActivity.STATE_EDIT:
						MyDBHelper mDbHelper = new MyDBHelper(ListAudioClipsActivity.this);
						...
						break;
					case FindActivity.STATE_INSERT:
						while (!mTempAudioMaps.isEmpty()) {
							String filename = Utils.getFilenameFromMediaUri(ListAudioClipsActivity.this,
									Uri.parse(mTempAudioMaps.getFirst().get(MyDBHelper.COLUMN_AUDIO_URI).toString()));
							File file = new File(filename);
							file.delete();
							mTempAudioMaps.removeFirst();
						}
						mTempAudioMaps.clear();
						...
						//create new intent to be passed onto recreation of this class
						intent = new Intent(ListAudioClipsActivity.this, ListAudioClipsActivity.class);
						intent.putExtra("rowId", getIntent().getLongExtra("rowId", -1));
						intent.putExtra("type", mState);
						intent.putExtra("length", mTempAudioMaps.size());
