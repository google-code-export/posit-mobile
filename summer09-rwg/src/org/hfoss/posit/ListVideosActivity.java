...
/**
 * Activity to list any videos associated with a specific find as a list adapter.
 * Clicking on any video in the list causes it to be played back.
 * @author Phil Fritzsche
 */
public class ListVideosActivity extends ListActivity implements ViewBinder, OnCreateContextMenuListener  {

	private static final String TAG = "ListVideosActivity";
	 ...
	 *  causing an error.
	 *  
	 *  Assign each and every item in the list to the context menu, so that when
	 *  the item is clicked and held, a menu will pop up.
	 */
	private int fillData() {
		Log.i(TAG, "filldata: refilling the data");
		mDbHelper = new MyDBHelper(this);

		String[] columns = MyDBHelper.list_video_row_data;
		int[] views = MyDBHelper.list_video_row_views;
		...
		if (mCursor.getCount() == 0) { // No videos
	...
	protected void onListItemClick(ListView listView, View view, int position, long id) {
		...
		Uri uri = mDbHelper.getVideoUriByPosition(getIntent().getLongExtra("rowId", -1), position);
	...
	public boolean onMenuItemSelected(int featureId, MenuItem item) {
		...
		case R.id.delete_all_videos:
			mDbHelper.close();
			showDialog(CONFIRM_DELETE_DIALOG);
			break;
		case R.id.cancel_delete_video_audio:
			Log.d(TAG, "NO VIDEO DELETED");
			return true;
	...
	/**
	 * Gives the user an option to either confirm a deletion or cancel it.
	 */
//	@Override
...
//		AdapterContextMenuInfo info = (AdapterContextMenuInfo) item.getMenuInfo();
//		switch (item.getItemId()) {
//		case R.id.cancel_delete_video:
//			Log.d(TAG, "NO VIDEO DELETED");
//			return true;
//		case R.id.confirm_delete_video:
//			MyDBHelper mDbHelper = new MyDBHelper(ListVideosActivity.this);
//			if (mDbHelper.deleteVideo(info.id)){
//				mDbHelper.close();
//				Utils.showToast(ListVideosActivity.this, R.string.deleted_from_database);
