...
int rwg_match_packetid(unsigned char packet_id[8],packetBuffer *packetBuff){
  ...
  for(;rc >= 0;rc--){ 
    ...
    if(memcmp(((rwg_header *)packetBuff->reqf[rc].reqf)->origin, packet_id, sizeof(((rwg_header *)packetBuff->reqf[rc].reqf)->origin)) == 0 && memcmp(&((rwg_header *)packetBuff->reqf[rc].reqf)->sequenceNumber,packet_id+6,sizeof(((rwg_header *)packetBuff->reqf[rc].reqf)->sequenceNumber)) == 0){   
      printf("rwg_match_packetid, rc: %i\n", rc);
      return rc;
    }   
  }
  return -1;
...
unsigned char* rwg_create_reqf_n(packetBuffer *packetBuff){
  ...
  for(;i < (sizeof(rwghdr->visited)/2);i++){
    rwghdr->visited[i] =  0x0;
    rwghdr->recentVisited[i] = 0x0;
  }

  /*hash n' add to recentVisited*/
  ...
  packetBuff->waiting[packetBuff->w_front % (sizeof((*packetBuff).waiting)/sizeof(wait_info))].reqf = (unsigned char *)rwghdr; 

  /*saves the position, that the reqf pointer is placed within the waiting buffer, in the reqf buffer*/
  packetBuff->reqf[packetBuff->reqf_counter].wait_pos = packetBuff->w_front % (sizeof((*packetBuff).waiting)/sizeof(wait_info)); 

  /*fetch a new time stamp*/
  t_stamp stamp;
  set_time_stamp(&stamp);

  packetBuff->waiting[packetBuff->w_front % (sizeof((*packetBuff).waiting)/sizeof(wait_info))].w_stamp = stamp;
  packetBuff->w_front++;

  /*make sure front wont reach the tail (when the buffers get full)*/
  if(packetBuff->w_front % (sizeof((*packetBuff).waiting)/4) == packetBuff->w_tail % (sizeof((*packetBuff).waiting)/sizeof(wait_info)) &&
     packetBuff->w_front != packetBuff->w_tail){
    packetBuff->waiting[packetBuff->w_tail % (sizeof((*packetBuff).waiting)/sizeof(wait_info))].reqf = NULL;
    packetBuff->w_tail++;
  }
...
unsigned char* rwg_create_reqf_f(packetBuffer *packetBuff){
  ...
  /*check if active_reqf is set*/
  if(packetBuff->active_reqf.reqf != NULL){
    ...
    /*stores the pointer to the reqf in the waiting buffer (reqfs that are waiting for acks)*/
    packetBuff->waiting[packetBuff->w_front % (sizeof((*packetBuff).waiting)/sizeof(wait_info))].reqf = (unsigned char *)packetBuff->active_reqf.reqf;   
    packetBuff->reqf[packetBuff->active_reqf.reqf_pos].wait_pos = packetBuff->w_front % (sizeof((*packetBuff).waiting)/sizeof(wait_info));
    packetBuff->waiting[packetBuff->w_front % (sizeof((*packetBuff).waiting)/sizeof(wait_info))].w_stamp = stamp;
    packetBuff->w_front++;

   
    /*make sure front wont reach the tail (when the buffers get full)*/
    if((packetBuff->w_front % (sizeof((*packetBuff).waiting)/sizeof(wait_info)) == packetBuff->w_tail % (sizeof((*packetBuff).waiting)/sizeof(wait_info))) &&
       packetBuff->w_front != packetBuff->w_tail){
      packetBuff->waiting[packetBuff->w_tail % (sizeof((*packetBuff).waiting)/sizeof(wait_info))].reqf = NULL;
      packetBuff->w_tail++;
    }
...
unsigned char* rwg_create_reqf_r(packetBuffer *packetBuff)
{
  ...
  if(reqf_c > -1){
	...
	return NULL;
      ...
      /*stores the pointer to the reqf in the waiting buffer (reqfs that are waiting for acks)*/
      packetBuff->waiting[packetBuff->w_front % (sizeof((*packetBuff).waiting)/sizeof(wait_info))].reqf = (unsigned char *)packetBuff->reqf[reqfPrevious].reqf;   
      packetBuff->waiting[packetBuff->w_front % (sizeof((*packetBuff).waiting)/sizeof(wait_info))].w_stamp = stamp;
      packetBuff->reqf[reqfPrevious].wait_pos = packetBuff->w_front % (sizeof((*packetBuff).waiting)/sizeof(wait_info));
      packetBuff->w_front++;
...
unsigned char* rwg_create_oktf(packetBuffer *packetBuff){
  ...
  if(packetBuff->w_tail == packetBuff->w_front){
  ...
  }else if(packetBuff->waiting[packetBuff->w_tail % (sizeof((*packetBuff).waiting)/sizeof(wait_info))].reqf == NULL){
    if(TRACE){printf("rwg_create_oktf, packetBuff->waiting[packetBuff->w_tail mod (sizeof((*packetBuff).waiting)/4)] is NULL \n");}
    packetBuff->w_tail++;
    return NULL;
  }else{
    packetBuff->active_reqf.reqf = packetBuff->waiting[packetBuff->w_tail % (sizeof((*packetBuff).waiting)/sizeof(wait_info))].reqf;
    ...
    memcpy(reqf_id,((rwg_header *)packetBuff->waiting[packetBuff->w_tail % (sizeof((*packetBuff).waiting)/sizeof(wait_info))].reqf)->origin,sizeof(unsigned char)*6);
    memcpy(reqf_id+sizeof(unsigned char)*6,&((rwg_header *)packetBuff->waiting[packetBuff->w_tail % (sizeof((*packetBuff).waiting)/sizeof(wait_info))].reqf)->sequenceNumber,sizeof(unsigned short int));
    ...
    if(rwghdr->hops > hops){
      ...
      int i = 0;    
      for(;i<(sizeof(rwghdr->recentVisited)/2);i++){
	rwghdr->recentVisited[i] = 0;
	((rwg_header *) packetBuff->active_reqf.reqf)->recentVisited[i] = 0;
      }
    }
  }
...
{ 
  ...
  if(SEND_ACK){
    ...
    if(TRACE){printf("SHOULD NOT GET HERE\n");}
    return 0;
  }
