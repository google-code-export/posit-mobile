...
public class ListAudioClipsActivity extends ListActivity implements OnCreateContextMenuListener {
	...
	private void fillDataFromDb() {
		...
		String[] columns = { MyDBHelper.KEY_AUDIO_URI };
	...
	private void populateMapList(int length) {	
		...
		for (int i = 0; i < length; i++) {
			...
			map.put(MyDBHelper.KEY_AUDIO_URI, getIntent().getStringExtra(String.valueOf(i)));
	...
	protected void onListItemClick(ListView listView, View view, int position, long id) {
		...
		case FindActivity.STATE_INSERT:
			...
			uri = Uri.parse(data.get(MyDBHelper.KEY_AUDIO_URI).toString());
			if (uri != null) {
				intent.setData(uri);
				startActivity(intent); 
			}
		...
		case R.id.confirm_delete_video_audio: //context menu call; delete specific file
			...
			case FindActivity.STATE_INSERT:
				...
				File file = new File(Utils.getFilenameFromMediaUri(this,
						Uri.parse(mTempAudioMaps.get(info.position).get(MyDBHelper.KEY_AUDIO_URI))));
				file.delete();
				...
				for (int i = 0; i < mTempAudioMaps.size(); i++) {
					resultIntent.putExtra(String.valueOf(i), mTempAudioMaps.get(i).get(MyDBHelper.KEY_AUDIO_URI).toString());
				}
				...
				intent.putExtra("length", mTempAudioMaps.size());
				for (int i = 0; i < mTempAudioMaps.size(); i++) {
					intent.putExtra(String.valueOf(i), mTempAudioMaps.get(i).get(MyDBHelper.KEY_AUDIO_URI).toString());
				}
					...
					case FindActivity.STATE_INSERT:
						while (!mTempAudioMaps.isEmpty()) {
							String filename = Utils.getFilenameFromMediaUri(ListAudioClipsActivity.this,
									Uri.parse(mTempAudioMaps.getFirst().get(MyDBHelper.KEY_AUDIO_URI).toString()));
